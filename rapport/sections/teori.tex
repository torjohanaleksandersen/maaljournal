...
Kondensatoren er en elektrisk komponent som kan lagre elektrisk energi i sitt dielektriske område. Komponenten har en viss kapasitans. Spenningen over kondensatoren bestemmes av formlen under:
\begin{equation}\label{eq:spenning_i_kondensator}
    v_C(t) = V_{F} + (V_{0} - V_{F})\cdot e^{-(t-t_0)/\tau}
\end{equation}
\noindent
$V_F$ er den endelige mulige spenningen kondensatoren kan oppnå og $V_0$ er startspenningen til kondensatoren. $\tau$ her vil være kondensatorstyrken ganget med totalresistansen sett fra kondensatoren, $\tau = R_{eq}\cdot C$. Basert på om det er en kilde i kretsen eller ikke, har vi en ladnings- og utladingsfase for kondensatoren. Vi kan generalisere formlene i hvert tilfelle. I ladingsfasen har vi initialbetingelser som gir formel:
\[
\begin{aligned}
    V_{F} &= V_{kilde} \\
    V_{0} &= 0 \\
...
Vi skal studere kretsen i to tidsintervaller hvor bryteren i node $D$ er i $A$ og $B$. Når bryteren er tilkoblet node $A$ vil kondensatoren lades gjennom batteriet $B_1$. Når bryteren er tilkoblet $B$ vil kondensatoren utlade gjennom motstand $R_2$. Vi vil studere denne kretsen i to tilfeller. Ladingsfase i tiden $0 \le t \le 10$, og en utladingsfase i tiden $10 \le t \le 20$. Dette skal vi gjøre med én kondensator, to kondensatorer i serie og to kondensatorer i parallell.


\subsubsection{Én kondensator $C_1$}
I dette tilfellet skal $D$ til $E$ kortsluttes og bryteren vil kobles til $A$ i tidsintervallet $t \in [0,10\text{s}]$ og med en gang etterpå tilkobles node $B$ i tidsintervallet $t \in [10\text{s}, 20\text{s}]$. Kretsen i figur \ref{fig:en_kond_figur} viser den overordnede kretsen.

\begin{figure}[H]\label{fig:en_kond_figur}
\centering
\begin{circuitikz}[american voltages]
...
\end{circuitikz}
\caption{Overordnet krets med én kondensator $C_1$.}
\end{figure}


\paragraph{Ladningsprosess $0 < t \le 10$.}
...
\end{circuitikz}
\caption{Kretsen i tidsintervallet $0 \le t \le 10$.}
\end{figure}
...
\]
\noindent
Vi løser for $V_C(10)$ for å vite hvilken spenningspotensial som er over kondensatoren ved $t=10\text{s}$. Dette blir startspenningen i utladingsfasen.
\begin{align}
...
\paragraph{Utladingsprosess $10 \le t \le 20$.} Her vil vi kortslutte node $D$ til $B$, kondensatoren lagrer nå en spenning og blir en spenningskilde som avtar her.
...
\end{circuitikz}
\caption{Kretsen i tidsintervallet $10 \le t \le 20$.}
\end{figure}
...
Nå har vi to uttrykk for før og etter bryterskiftet. Denne funksjonen beskriver spenning over $DF$ på kretsen i figur~\ref{fig:en_kond_figur} i tidsintervallet $0 \le t \le 20$. Dette er da ligning~\ref{eq:oppgaveAFør10} og ligning~\ref{eq:oppgaveAEtter10} med sine respektive tider:
\begin{equation}
V_C(t) =
\begin{cases}
10 \left(1 -  e^{-t/0.816}\right), & 0 \le t \le 10, \\
10 e^{-(t-10)/1.836} & 10 < t \le 20.
\end{cases}
\end{equation}


\subsubsection{To kondensatorer i seriekobling}\label{subsec:tilfelle2iSerie}
Her er kretsen i figur~\ref{fig:Krets} modellert slik at det vil være en ny kondensator mellom node $E$ og $D$. Den har samme verdi som $C_1$, men navngitt $C_2$. Kretsen blir seende slik ut nå:
...
\end{circuitikz}
\caption{To kondensatorer i seriekobling. }
\end{figure}

\noindent
I denne kretsen kan vi beskrive de ulike spenningene over $DF$. Det vil være én spenning over $C_1$, én over $C_2$ og vi kan også beskrive spenningen over hele $DF$. Siden disse er i seriekobling, strømmen vil være like over $DF$, og både $C_1$ og $C_2$ er like kan vi se at spenningen over begge kondensatorene må være like.
\[
\begin{aligned}
...
\paragraph{Ladingsprosess $0 \le t \le 10$. } Her har vi en kortslutting fra $D$ til $A$.
...
\end{circuitikz}
\caption{To kondensatorer i seriekobling i tiden $0 \le t \le 10$.}
\end{figure}
...
\paragraph{Utladingsprosess $10 \le t \le 20$. } Her vil vi kortslutte fra $D$ til $B$ og forenkle kretsen som under.
\begin{figure}[H]
\centering
\begin{circuitikz}[american voltages]
...
\end{circuitikz}
\caption{To kondensatorer i seriekobling i tiden $10 \le t \le 20$.}
\end{figure}



\noindent
Utregning for $V_{DF}(t)$ innebærer å vite $V_{DF}(10)$ i ladingsprosessen. Denne er da $\left(\approx 10\right)$. $V_{DF}(t)$ for denne utladeprosessen er som under.
\[
\begin{aligned}
    \tau &= R_2 C_{tot} \\
...
\end{circuitikz}
\caption{Parallellkoblede kondensatorer $C_1$ og $C_2$}
\end{figure}
...
\end{circuitikz}
\caption{Parallellkoblede kondensatorer i tiden $0 \le t \le 10\,\text{s}$}
\end{figure}
...
\begin{aligned}
    ...
    \tau &= 1.632\,\text{s}, \\[0.5em]
    V_{DF}(t) &= V_B\bigl(1 -  e^(-t/\tau)\bigr) \\
    V_{DF}(t) &= 10\bigl(1 -  e^(-t/1.632)\bigr), \qquad 0 \le t \le 10.
\end{aligned}
\]
...
\end{circuitikz}
\caption{Parallellkoblede kondensatorer $C_1$ og $C_2$ i tiden $10\text{s} \le t \le 20 \text{s}$}
\end{figure}
...
\begin{aligned}
    ...
    \tau &= 3.672\,\text{s}, \\[0.5em]
    V_{DF}(t) &= V_{DF}(10)\, e^{-(t-10)/\tau} \\
    V_{DF}(t) &\approx 10\, e^{-(t-10)/3.672}, \qquad 10 \le t \le 20.
\end{aligned}
\]
...
V_{DF}(t) =
\begin{cases}
10\bigl(1 -  e^{-t/1.632}\bigr), & 0 \le t \le 10, \\
10 e^{-(t-10)/3.672}, & 10 \le t \le 20,
\end{cases}
\]
...
Vi har tre tilfeller. Tilfelle 1-3 er; én alene, to i seriekobling og to i parallellkobling. Grafing er under.
...
\[
\begin{aligned}
    v_C(t)&=v_C(9\text{ms}) e^ {-(t-9\text{ms})/\left(1.69\text{ms}\right)} \\
