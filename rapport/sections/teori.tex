...
\noindent
I utladingsfasen har vi andre initialbetingelser og vi kan utlede en formel for $v_C(t)$ i denne fasen. Her vil $t_0$ være tiden brukt i oppladingsfasen, startspenning være spenningen oppnådd i oppladingsfasen og endelig spenning vil være lik null.
...
\begin{circuitikz}[american voltages]
    ...
    \node[circle, fill=black, inner sep=0pt] (G)  at (0,0)  {};
    \node[circle, fill=black, inner sep=0pt] (T1) at (0,5)  {}; % etter batteri oppe

    % Øvre gren
    \node[circle, fill=black, inner sep=1.2pt] (A)  at (4,5)  {};
    \node[circle, fill=black, inner sep=1.2pt] (B)  at (6,5)  {};
    \node[circle, fill=black, inner sep=0pt] (T2) at (10,5) {};

    % Nedre gren
    \node[circle, fill=black, inner sep=0pt] (BR) at (10,0) {};
    ...
    \draw (F) to[C, l_=$C_1$, a^=$68\text{\textmu F}$] (E);
...
\end{circuitikz}
\caption{RC krets med én kondensator}\label{fig:en_kond_figur}
\end{figure}
...
\begin{circuitikz}[american voltages]
    ...
    % Nedre gren
    \node[circle, fill=black, inner sep=0pt] (F)  at (5,0)  {};

    \node[circle, fill=black, inner sep=0pt] (D)  at (5,5)  {};
    ...
    %--- Kondensator C1 mellom E og F ----------------------------
    \draw (F) to[C, l_=$C_1$, a^=$68\text{\textmu F}$] (D);
...
\begin{align*}
    ...
    V_F &= B_1 = 10\text{V} \\
    \tau &= R_1C \\
    \tau &= 12\text{k}\Omega \cdot 68\text{\textmu F} \\
    \tau &= 0.816\text{s}  
\end{align*}


\noindent
Nå kan vi utlede et uttrykk for oppladingsfasen og vi løser for $V_C(10)$ for å vite hvor mye spenning som kondensatoren har lagret ved starten av utladingsfasen.

\begin{align}
    V_C(t) &= 10 \left(1 -  e^{-t/0.816}\right) \label{eq:oppgaveAFør10} \\
    V_C(10) &= 10 \left(1 -  e^{-12.25}\right) \nonumber \\
    V_C(10) &\approx 10\text{V} \nonumber
\end{align}
...
\begin{aligned}
    ...
    V_{0} &= 10V \\
    \tau &= R_2C \\
    \tau &= 27\text{k}\Omega \cdot 68\text{\textmu F} \\
    \tau &= 1.836s
\end{aligned}
\]
\begin{equation}
...
\begin{circuitikz}[american voltages]
    ...
    \draw (E) to[C, l_=$C_2$, a^=$68\text{\textmu F}$] (D);
...
\vspace{1em}
\noindent
Dermed kan vi konkludere med at spenningene må være like.
...
\[
\begin{aligned}
    \phantom{= v_{C}(t)} v_{C1}(t) &= v_{C2}(t) = v_{C}(t) \\
...
\vspace{1em}
\noindent
Nå skal vi analysere kretsen i lade- og utladingsprosessen. For å kunne bruke spenning i kondensatorformlen må vi behandle $C_1$ og $C_2$ som én totalkapasitator. Da må vi finne totalkapasitansen sett fra $DF$. $C_{tot}$ blir: $\left(C_{tot}\right)^{-1} = \left(C_1\right)^{-1} + \left(C_2\right)^{-1} \rightarrow C_{tot} = 34 \text{\textmu F}$.
...
\begin{circuitikz}[american voltages]
    ...
    \node[circle, fill=black, inner sep=0pt] (E)  at (5, 2.5)  {};
...
\begin{aligned}
    \tau &= R_1 C_{tot} \\
    \tau &= 12\text{k}\Omega \cdot 34\text{\textmu F} \\
    \tau &= 0.408\text{s} \\
    V_{DF}(t) &= V_B (1 -  e^ {-t/\tau}) \\
    V_{DF}(t) &= 10 (1 -  e^ {-t/0.408}) \\
    \phantom{e^ {-t/0.408}}V_{DF}(10) &\approx 10\text{V}
\end{aligned}
\]
...
\begin{aligned}
    \tau &= R_2 C_{tot} \\
    \tau &= 27\text{k}\Omega \cdot 34\text{\textmu F} \\
    \tau &= 0.918\text{s} \\
    V_{DF}(t) &= 10  e^ {-(t - 10)/0.918}
\end{aligned}
\]

\paragraph{Endelig uttrykk. } Med bakgrunn på uttrykkene i oppladings- og utladingsfasen kan vi uttrykke to uttryk for hele perioden. Et uttrykk som beskriver totalspenningen over området $DF$ og et uttrykk for hver enkelt kondensator.
...
\begin{circuitikz}[american voltages]
    ...
    \node[circle, fill=black, inner sep=0pt] (BR) at (10,0) {};
    \node[circle, fill=black, inner sep=1.2pt] (F)  at (5,0)  {};
    \node[circle, fill=black, inner sep=0pt] (FL)  at (3,0)  {};
    \node[circle, fill=black, inner sep=0pt] (FR)  at (7,0)  {};
    \node[circle, fill=black, inner sep=1.2pt] (E)  at (5,2)  {};
    \node[circle, fill=black, inner sep=0pt] (EL)  at (3,2)  {};
    \node[circle, fill=black, inner sep=0pt] (ER)  at (7,2)  {};
    ...
    %--- Kondensator C1 mellom E og F ----------------------------
    \draw (FL) to[C, l_=$C_1$, a^=$68\text{\textmu F}$] (EL);
    \draw (FR) to[C, l_=$C_2$, a^=$68\text{\textmu F}$] (ER);
...
\[
C_{tot} = C_1 + C_2 \;\;\Rightarrow\;\; C_{tot} = 136\text{\textmu F}.
\]
...
\begin{aligned}
    \tau &= R_1 C_{tot} \\
    \tau &= 12\text{k}\Omega \cdot 136\text{\textmu F} \\
    \tau &= 1.632\text{s} \\
...
\noindent
Når $t = 10\text{s}$ er det gått flere tidskonstanter, og vi kan ta $V_{DF}(10) \approx 10\text{V}$.
...
\begin{aligned}
    \tau &= R_2 C_{tot} \\
    \tau &= 27\text{k}\Omega \cdot 136\text{\textmu F} \\
    \tau &= 3.672\text{s} \\
...
Vi har tre tilfeller. Tilfelle 1-3 for kondensatorer er; én alene, to i seriekobling og to i parallellkobling. Grafen er vist under.

\begin{figure}[H]
\centering
\begin{tikzpicture}\label{fig:teoretisk_tilfellene_1til3}
\begin{axis}[
    width=0.9\textwidth,
    height=7cm,
    xlabel={$t[\mathrm{s}]$},
...
\end{axis}
\end{tikzpicture}
\caption{Inngangssignal $v(t)$ med høy-nivå $8\mathrm{V}$ i 9ms per periode.}
\label{fig:grafet_v_av_t}
\end{figure}
...
\begin{circuitikz}[american voltages]
    ...
    \node[circle, fill=black, inner sep=0pt] (BL)  at (0, 0)  {};
    \node[circle, fill=black, inner sep=1.2pt] (B)  at (5, 0)  {};
    \node[circle, fill=black, inner sep=0pt] (BR)  at (10, 0)  {};
    \node[circle, fill=black, inner sep=0pt] (TL)  at (0, 5)  {};
    \node[circle, fill=black, inner sep=1.2pt] (T)  at (5, 5)  {};
    \node[circle, fill=black, inner sep=0pt] (TR)  at (10, 5)  {};
    ...
    \draw (T) to[C, l_=$v_C(t)$, a^=$1\text{\textmu F}$] (B);
    \draw (BL) -- (BR);
...
\[
\begin{aligned}
    \phantom{123456789123} R_{eq} &= 2.2\text{k}\Omega || (3.9+3.3)\text{k}\Omega \\
    R_{eq} &= \frac{2.2\cdot7.2}{2.2+7.2}\text{k}\Omega \\
    R_{eq} &\approx 1.69\text{k}\Omega \\
    \tau &= R_{eq}C \approx 1.69\text{k}\Omega \cdot 1 \text{\textmu F} \\
...
\begin{aligned}
    ...
    v_C(9\text{ms})&=6.07\text{V} \\
    v_C(t) &=6.07 \cdot e^{-(t-9\text{ms})/\left(1.69\text{ms}\right)}
\end{aligned}
\]
...
\label{fig:teor_vC_ogv0}
\end{figure}


\subsubsection{Oppladningstida og utladingstida for $C$}
